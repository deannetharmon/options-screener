...
export async function GET() {
  try {
    ...
    // Use first account (or the one marked as primary)
    ...
    // Filter to options only (instrument-type = 'Equity Option' or 'Index Option')
    const optionPositions = rawPositions.filter((p: any) =>
      p['instrument-type'] === 'Equity Option' || p['instrument-type'] === 'Index Option'
    );

    // Group legs by underlying symbol + expiration to reconstruct spreads
    ...
    // Fetch current market prices for all option symbols to calculate P&L
    ...
    if (allOptionSymbols.length > 0) {
      ...
      } catch { /* prices optional — P&L will show as N/A */ }
    }

    // Build normalized position objects
    ...
    const positions = Object.entries(groups).map(([key, legs]) => {
      ...
      // Infer strategy from legs
      const putLegs = legs.filter(l => l['option-type'] === 'P');
      const callLegs = legs.filter(l => l['option-type'] === 'C');
      ...
      // Calculate credit received (sum of average open prices, short legs positive)
      let creditReceived = 0;
      for (const leg of legs) {
        const qty = parseInt(leg['quantity'] ?? '1', 10);
        const avgPrice = parseFloat(leg['average-open-price'] ?? '0');
        const direction = leg['quantity-direction']; // 'Short' or 'Long'
        creditReceived += direction === 'Short' ? avgPrice * qty : -(avgPrice * qty);
      }
      creditReceived = creditReceived * 100; // convert to dollar value
      ...
      for (const leg of legs) {
        ...
        if (price == null) { hasCurrentPrices = false; continue; }
        const direction = leg['quantity-direction'];
        currentValue += direction === 'Short' ? price * qty : -(price * qty);
      }
      currentValue = currentValue * 100;

      const pnl = hasCurrentPrices ? creditReceived - (currentValue * -1 + creditReceived) : null;
      const pnlPct = creditReceived !== 0 && pnl != null ? (pnl / Math.abs(creditReceived)) * 100 : null;
      const targetPrice = Math.abs(creditReceived) * 0.5;
      const hitTarget = hasCurrentPrices && Math.abs(currentValue) <= targetPrice;
    ...
    // Sort: needs close first, then by DTE ascending
    positions.sort((a, b) => {
