...
// ── Saved Filters API ──────────────────────────────────────────────────────
async function loadFilters(strategy: string): Promise<SavedFilters> {
  try {
...
async function saveFilter(strategy: string, name: string, tickers: string[], replace = false): Promise<{ success?: boolean; conflict?: boolean; message?: string }> {
  const res = await fetch('/api/filters', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ strategy, name, tickers, replace }),
  });
  return res.json();
}
...
async function extractTickersFromImage(file: File): Promise<string[]> {
  ...
  const blacklist = new Set(['USA','ETF','CEO','IPO','NYSE','NASDAQ','OTC','ADR','INC','LLC','LTD','PLC','THE','AND','FOR','REQ','BPS','BCS','PUT','CALL','OTM','ITM','ATM','IVR','DTE','ROC','POP','GTC','OCO','A','I','S','P','C','B','N','E']);
...
async function getAccessToken(): Promise<string> {
  ...
  const res = await fetch(`${BASE}/oauth/token`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ grant_type: 'refresh_token', refresh_token: refreshToken.trim(), client_id: clientId.trim(), client_secret: clientSecret.trim() }) });
...
function findBestSpread(chain: any[], strategy: 'BPS' | 'BCS', expDate: string, price: number | null, RULES: RulesType): SpreadCandidate | null {
  ...
  for (const width of getWidthSteps(RULES.MAX_SPREAD_WIDTH, price)) {
    ...
    if (c && (best === null || c.roc > best.roc)) best = c;
  }
  return best;
}
...
function runChecklist(symbol: string, strategy: 'BPS' | 'BCS' | 'IC', metrics: any, chainData: { expirations: string[]; chains: Record<string, any[]> }, price: number | null, RULES: RulesType, trendResult?: TrendResult): ScreenResult {
  ...
  if (!earningsDate) { earningsCheck = { status: 'pass', value: 'None found', reason: 'Safe to trade' }; }
    ...
    if (daysAway < 0) { earningsCheck = { status: 'pass', value: `${earningsDate} (past)`, reason: 'Already reported' }; }
    else if (daysAway < 30) { failReasons.push(`Earnings in ${daysAway}d`); earningsCheck = { status: 'fail', value: `${daysAway}d (${earningsDate})`, reason: 'Within expiry window' }; }
    else { earningsCheck = { status: 'pass', value: `${daysAway}d (${earningsDate})`, reason: 'Outside expiry window' }; }
  }
  const validExpirations = chainData.expirations.filter(exp => {
    ...
    if (earningsDate) { const ed = daysUntil(earningsDate); if (ed >= 0 && ed <= dte) return false; }
    return true;
  });
...
function StrategyBox({ label, badge, badgeColor, borderFocus, value, onChange, strategy, disabled }: StrategyBoxProps) {
  ...
  const refreshFilters = useCallback(async () => {
    ...
    const f = await loadFilters(strategy);
    setSavedFilters(f);
  ...
  const handleOCR = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (tickers.length > 0) onChange(appendTickers(value, tickers));
  ...
  const handleSave = async (replace = false) => {
    ...
    if (result.conflict) {
      setSaveError(`"${saveName}" exists — replace?`);
      return;
    }
  ...
  const handleLoad = (name: string) => {
    ...
    onChange(appendTickers(value, tickers));
    setShowLoad(false);
  };
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          <span className={`text-[9px] px-1.5 py-0.5 rounded tracking-wider border ${badgeColor}`}>{badge}</span>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {/* Save */}
          <button onClick={() => { setShowSaveInput(!showSaveInput); setShowLoad(false); setSaveError(''); }}
            disabled={disabled}
            className="text-[9px] px-1.5 py-0.5 border border-slate-600 rounded text-slate-300 hover:border-slate-400 hover:text-white transition-colors disabled:opacity-40"
          ...
          {/* Load */}
          <button onClick={() => { setShowLoad(!showLoad); setShowSaveInput(false); if (!showLoad) refreshFilters(); }}
            disabled={disabled}
            className="text-[9px] px-1.5 py-0.5 border border-slate-600 rounded text-slate-300 hover:border-slate-400 hover:text-white transition-colors disabled:opacity-40"
      ...
      {showSaveInput && (
        <div className="mb-1.5 flex flex-col gap-1">
          <div className="flex gap-1">
            <input
              type="text" value={saveName} onChange={e => { setSaveName(e.target.value); setSaveError(''); }}
            ...
            />
            <button onClick={() => handleSave()} className="text-[9px] px-2 py-1 bg-slate-700 hover:bg-slate-600 text-white rounded transition-colors">Save</button>
          </div>
          {saveError && (
            <div className="flex gap-1 items-center">
              <span className="text-[9px] text-yellow-400">{saveError}</span>
              {saveError.includes('exists') && (
                <button onClick={() => handleSave(true)} className="text-[9px] px-1.5 py-0.5 bg-yellow-700 hover:bg-yellow-600 text-white rounded">Replace</button>
              )}
            </div>
          )}
        </div>
      ...
      {showLoad && (
        <div className="mb-1.5 bg-slate-800 border border-slate-600 rounded overflow-hidden">
          {loadingFilters ? (
            <p className="text-[9px] text-slate-400 px-2 py-1.5">Loading...</p>
          ) : filterNames.length === 0 ? (
            <p className="text-[9px] text-slate-500 px-2 py-1.5">No saved filters</p>
          ) : (
            filterNames.map(name => (
              <div key={name} className="flex items-center justify-between px-2 py-1 hover:bg-slate-700 group">
                <button onClick={() => handleLoad(name)} className="text-[10px] text-slate-200 hover:text-white text-left flex-1">{name}</button>
    ...
    <div className={`border rounded-lg overflow-hidden cursor-pointer transition-all ${result.qualified ? 'border-slate-600 bg-slate-900/60' : 'border-slate-700 bg-slate-900/30 opacity-60'}`} onClick={() => setExpanded(!expanded)}>
      <div className="px-4 py-3 flex items-center gap-3 flex-wrap">
        <div className="w-16 shrink-0">
          <p className="font-bold text-white text-sm">{result.symbol}</p>
          {result.price && <p className="text-[10px] text-slate-300">${result.price.toFixed(2)}</p>}
        </div>
      ...
      {expanded && (
        <div className="border-t border-slate-700 px-4 py-3 space-y-3">
          ...
          {c && c.strategy === 'IC' && c.callWidth != null && c.callWidth !== c.spreadWidth && <div className="pt-2 border-t border-slate-700"><p className="text-[10px] text-slate-300">Asymmetric widths — Put: ${c.spreadWidth} · Call: ${c.callWidth} (each optimized for best ROC)</p></div>}
      ...
      <div className="flex h-[calc(100vh-57px)]">
        ...
        <div className="w-80 border-r border-slate-700 p-4 overflow-auto flex flex-col gap-4 shrink-0">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                <span className="text-[9px] px-1.5 py-0.5 bg-purple-900/40 text-purple-300 border border-purple-700 rounded tracking-wider">AUTO</span>
            ...
            {autoOverLimit && <p className="text-[9px] text-red-400 mt-1 font-medium">Max {AUTO_TICKER_LIMIT} tickers (free tier limit)</p>}
          ...
          <div className="border-t border-slate-700 pt-3 space-y-4">
            ...
            <StrategyBox label="BPS" badge="BULLISH" badgeColor="bg-emerald-900/40 text-emerald-300 border-emerald-700" borderFocus="focus:border-emerald-600" value={bpsTickers} onChange={handleBpsChange} strategy="BPS" disabled={loading} />
            <StrategyBox label="BCS" badge="BEARISH" badgeColor="bg-red-900/40 text-red-300 border-red-700" borderFocus="focus:border-red-600" value={bcsTickers} onChange={handleBcsChange} strategy="BCS" disabled={loading} />
            <StrategyBox label="IC" badge="NEUTRAL" badgeColor="bg-blue-900/40 text-blue-300 border-blue-700" borderFocus="focus:border-blue-600" value={icTickers} onChange={handleIcChange} strategy="IC" disabled={loading} />
          </div>

          {/* Active Rules */}
          <div className="text-[9px] space-y-1 border-t border-slate-700 pt-3">
            <p className="text-slate-300 mb-2 tracking-widest text-[9px] font-medium">ACTIVE RULES</p>
            {[
        ...
        <div className="flex-1 overflow-auto p-5">
          {results.length === 0 && !loading && (
            <div className="h-full flex flex-col items-center justify-center text-slate-500">
              ...
              <p className="text-[9px] mt-2 text-slate-500">Upload a Finviz screenshot · Save & load ticker lists</p>
            </div>
          )}
          {loading && (
            <div className="h-full flex flex-col items-center justify-center gap-2">
              <div className="text-[10px] tracking-widest text-slate-300 animate-pulse font-medium">{status || 'SCANNING...'}</div>
            </div>
          )}
          {results.length > 0 && (
            <div className="space-y-5">
              ...
              {disqualified.length > 0 && (
                <div>
                  <p className="text-[9px] text-slate-500 tracking-widest mb-2 font-medium">DISQUALIFIED</p>
