...
async function fetchAndReconstructTrades(range: TimeRange): Promise<ClosedTrade[]> {
  ...
  for (const [sym, txList] of Object.entries(byOptionSymbol)) {
    ...
    const opens  = txList.filter((tx: any) => tx['action-description'] === 'Sell to Open' || tx['action-description'] === 'Buy to Open');
    const closes = txList.filter((tx: any) => tx['action-description'] === 'Buy to Close' || tx['action-description'] === 'Sell to Close');
      ...
      <div className={`${th.header} border-b ${th.border} px-6 py-4 flex items-center justify-between sticky top-0 z-50`}>
        <div className="flex items-center gap-6">
          ...
          <nav className="flex items-center gap-1 bg-black/20 rounded-lg p-1">
            ...
            <span                     className="text-xs px-3 py-1.5 rounded text-white tracking-wider active-nav" style={{ backgroundColor: `rgba(var(--accent-r),var(--accent-g),var(--accent-b),0.25)`, borderBottom: `2px solid var(--accent)` }}>TRADE LOG</span>
